...
def run_simulation_transformer(seed, NUM_CLIENTS, defense_nodes, malicious_clients, G, neighbors, client_datasets, test_data,
                           ...
                           debug_mode=False, GLOBAL_ROUNDS=15, norm_factor=40, debug=True, epochs=1, MAX_WORKERS=8,  audit_prob=0.9,
                            agg_prob=0.8, agg_threshold=0.4,
                           log_file_path=None):
    ...
    if mechanism == 'MAB':
      #  mab_defense = MABDefense_transformer(NUM_CLIENTS)
        mab_defense =  MABDefense(NUM_CLIENTS, model_type='transformer',audit_prob=audit_prob, agg_prob=agg_prob, agg_threshold=agg_threshold)
    # Config
